...
let defaultColor;
let hoverColor;
let textDefaultColor; // Default text color
let textHoverColor; // Hover text color
let shadowOffsetX;
let shadowOffsetY;
let shadowBlur;
let shadowColor;
...
// Declare border radius variables
...
function setup() {
    ...
    textDefaultColor = buttonContainerElt.getAttribute('data-text-color-default'); // Default text color
    textHoverColor = buttonContainerElt.getAttribute('data-text-color-hover'); // Hover text color

    // Initialize shadow variables
    shadowOffsetX = parseInt(buttonContainerElt.getAttribute('data-shadow-offset-x'));
    shadowOffsetY = parseInt(buttonContainerElt.getAttribute('data-shadow-offset-y'));
    shadowBlur = parseInt(buttonContainerElt.getAttribute('data-shadow-blur'));
    shadowColor = buttonContainerElt.getAttribute('data-shadow-color-default');
    hoverShadowColor = buttonContainerElt.getAttribute('data-shadow-color-hover');

    // Initialize border radius variables
    borderRadiusTopLeft = parseInt(buttonContainerElt.getAttribute('data-border-radius-top-left')) || 0;
    borderRadiusTopRight = parseInt(buttonContainerElt.getAttribute('data-border-radius-top-right')) || 0;
    borderRadiusBottomRight = parseInt(buttonContainerElt.getAttribute('data-border-radius-bottom-right')) || 0;
    borderRadiusBottomLeft = parseInt(buttonContainerElt.getAttribute('data-border-radius-bottom-left')) || 0;
    ...
    vertices = [
        { x: 5, y: 5, radius: borderRadiusTopLeft },
        { x: buttonWidth - borderRadiusTopRight, y: 5, radius: borderRadiusTopRight },
        { x: buttonWidth, y: 5 + buttonHeight * 0.4, radius: borderRadiusBottomRight },
        { x: buttonWidth * 0.85, y: buttonHeight, radius: borderRadiusBottomRight },
        { x: 5, y: buttonHeight, radius: borderRadiusBottomLeft },
    ];
...
function drawButton() {
    ...
    fill(isHovered ? textHoverColor : textDefaultColor); 
    textAlign(CENTER, CENTER);
    textSize(buttonHeight / 3.5);
    text(buttonText.innerHTML, buttonWidth / 2, buttonHeight / 2); // Positioning text
